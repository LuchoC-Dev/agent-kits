...
# kits-init — Lanzador del agente de inicialización

## Qué es esta skill

Es un **lanzador delgado** (thin launcher). No contiene el procedimiento de
inicialización: su única responsabilidad es **lanzar el agente `kits-init`** en su propio
...
3. **Seguí las instrucciones de `agent.md` como el agente `kits-init`, en esta misma
...
- No actúes proactivamente fuera del trigger `/kits-init`.
